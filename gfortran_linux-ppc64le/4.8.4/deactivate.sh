    ...
    for thing in $tc_nature,$tc_machine "$@"; do
      case "${thing}" in
        *,*)
          ...
          ;;
        *)
          newval=$(which ${cc_prefix}/bin/${tc_prefix}${thing} 2>/dev/null)
          if [ -z "${newval}" -a "${pass}" = "check" ]; then
            echo "ERROR: This cross-compiler package contains no program ${cc_prefix}/bin/${tc_prefix}${thing}"
            return 1
          fi
          ;;
      esac
...
env > /tmp/old-env-$$.txt
_tc_activation \
  deactivate host powerpc64le-linux-gnu "" \
  c++ g++ \
  "CXXFLAGS,${CXXFLAGS:-}" \
