...
MAKE_JOBS=${BB_MAKE_JOBS}
[[ ${MAKE_JOBS} -gt 0 ]] 2>/dev/null || \
    { echo "MAKE_JOBS must be integer > 0." >&2; exit 1; }
...
make -j${MAKE_JOBS} chrysalis_target TRINITY_COMPILER=gnu \
...
# samtools is from the 1.x branch and not the 0.9.x one Trinity uses.
