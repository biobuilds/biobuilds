...
if [ "$build_arch" == "ppc64le" ]; then
    ...
    # S82xL servers support a max of 2 12-core, SMT8-enabled CPUs.
    MAX_THREADS=192
    ...
    # Build a version of "DYNAMIC_ARCH" library for x86_64 with certain (e.g.,
    # mobile or older than Nehalem) microarchitectures removed. This lets users
    # benefit from better optimizations if their CPUs support them, while
    # reducing the size of the conda package we produce. For comparison:
    ...
    if [ "$build_os" == "Darwin" ]; then
        # Theoretical max would be 24 (12-core Xeon, HT enabled) for very
        # top-end Mac Pro but more reasonable to assume we'll be running on
        # more common 2- or 4-core, HT-enabled systems.
        MAX_THREADS=8
...
#
# NOTE: Must compile with NO_AFFINITY=1. Without it, compatibility with
# things like R and NumPy breaks in a very, very bad way.
#
make -j${BB_MAKE_JOBS} \
